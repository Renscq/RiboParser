...
```bash
cd ./sce/6.serp/05.metaplot
```

Draw TIS/TTS enrichment metaplots from the merged RPF density file with matched control/IP samples, writing both PDF and PNG figures.

```bash
serp_metaplot \
  -r ../../../4.ribo-seq/5.riboparser/05.merge/sce_rpf_merged.jsonl.gz \
