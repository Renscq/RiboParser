...
# Date: 2026-06-12
# Version: 0.2.7
# Function: This script is used to merge the quant files.
...
def import_quant_files(quant_list):
    ...
    -------
    object
        output dataframe contain the quant.

    Notes
    -----
    Legacy input information:
    - ``list``: pattern for quant files (e.g., '*_rpf_quant.txt').

    Workflow
    --------
    1. Retrieve the quant.
    """
...
def output_table(quant_df, output_file):
    ...
    -----
    Legacy input information:
    - ``result_dict``: nested dict contain the quant.
