...
# Date: 2026-06-12
# Version: 0.2.7
# Function: This script is used to merge the length distribution files.
...
def process_log_files(length_file_list):
    ...
    Parameters
    ----------
    length_file_list : object
        Input argument used by this function.

    Returns
    -------
    object
        output nested dict contain the reads length.

    Notes
    -----
    Legacy input information:
    - ``list``: pattern for reads length files (e.g., '*length_distribution.txt').

    Workflow
    --------
    1. Retrieve the reads length.
    """
...
def output_table(reads_length_df, output_file):
    ...
    -----
    Legacy input information:
    - ``result_dict``: nested dict contain the mapped reads.
