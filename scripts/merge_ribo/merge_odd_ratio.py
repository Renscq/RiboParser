...
# Date: 2026-06-12
# Version: 0.2.7
# Function: This script is used to merge the codon odd ratio files.
...
def import_odd_ratio_files(odd_ratio_list):
    """Retrieve the codon odd ratio table.

    Parameters
    ----------
    odd_ratio_list : object
        Input argument used by this function.

    Returns
    -------
    object
        output dataframe contain the odd ratio.

    Notes
    -----
    Legacy input information:
    - ``list``: pattern for codon odd ratio files (e.g., '*_sum_codon_odd_ratio.txt').

    Workflow
    --------
    1. Retrieve the codon odd ratio.
    """
...
def output_table(odd_ratio_merge, output_file):
    ...
    -----
    Legacy input information:
    - ``result_dict``: nested dict contain the codon odd ratio.
