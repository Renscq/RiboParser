...
class CodonSelectiveTime(object):
    ...
    def mrna_codon_frequency(self, rpf_sample=None, rna_sample=None, times=0, initiate_rate=None):
        ...
        if initiate_rate:
            # fix the bugs of polars map_elements without return_dtype

            gene_dict = {k: v.initiate_rate for k, v in self.gene_table[rpf_sample].items()}

            high_rna = self.high_rna.with_columns(
                (pl.col(rna_sample) *
                pl.col("name").map_elements(lambda x: gene_dict.get(x, 1), return_dtype=pl.Float32)
                ).alias(rna_sample + "_adj")
            )
    ...
    def draw_cst_line(self):
        ...
        sns.lineplot(data=rel_cst, x="iteration", y="cst", estimator=None)  # estimator=None keep the raw point
