...
# Date: 2026-06-12
# Version: 0.2.7
# Function: Provide RiboParser core functions for Coefficient of Variation analysis.
...
class CoV(object):
    ...
    def test_group_CoV(self):
        ...
        - ``self.group``: the group file.
        - ``self.re_combination``: the function to reterieve the comparasion.

        Legacy return information:
        - ``self.test_CoV_dict``: the dict contain the CoV of each comparasion.
        - ``self.test_CoV``: the dataframe contain the CoV of each comparasion.

        Workflow
        --------
        1. Reterieve the comparasion.
        2. Reterieve the samples and CoV.
        3. Calculate the differential pvalue with ks-test and student's-test.
        4. Output the result.
        """
    ...
    def draw_fit_plot(self):
        ...
        -------
        object
            the pdf file of fit plot the dataframe contain the group information.

        Notes
        -----
        Legacy input information:
        - ``self.merge_file``: the CoV merge file.

        Workflow
        --------
        1. Split the group to each comparasion.
        2. Reterieve the samples and CoV.
        3. Create the figure with 3 columns and 1 row, containing 3 subplots.
        - first subplot --> draw the scatter and fitted line plot for group1, x =
        log2(mean), y = log2(CoV).
        - second subplot --> draw the scatter and fitted line plot for group2, x =
        log2(mean), y = log2(CoV).
        - third subplot --> draw the fitted line plot for group1 and group2, x =
        log2(mean), y = log2(CoV).
        4. Draw the scatter and fitted line plot for each group, x = log2(mean), y =
        log2(CoV).
        5. Draw the fitted line plot for each group, x = log2(mean), y = log2(CoV).
        6. Output the result.
        """
