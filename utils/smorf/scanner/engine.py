...
from .reference import GENETIC_CODE, STOP_CODONS, CoordinateMapper, SeqUtils
...
class ORFScanner:
    ...
    def __init__(
        ...
        if invalid_start_codons:
            raise ValueError("Invalid start codon(s): " + ", ".join(invalid_start_codons))

        stop_as_start = sorted(normalized_start_codons.intersection(STOP_CODONS))
        if stop_as_start:
            raise ValueError(
                "Stop codons cannot be used as start codons: " + ", ".join(stop_as_start)
            )
        if min_aa < 1:
        ...
        if scan_strand not in VALID_SCAN_STRANDS:
            raise ValueError("scan_strand must be one of: sense, antisense, both.")
    ...
    def scan_transcript(self, tx: Transcript) -> list[ORFRecord]:
        ...
        if not tx.tx_seq:
            raise ValueError(f"Transcript sequence has not been reconstructed: {tx.transcript_id}")
        if len(tx.tx_seq) != tx.transcript_length():
            raise ValueError(f"Transcript sequence length mismatch for {tx.transcript_id}.")
        ...
        for frame in range(3):
            ...
            for position in range(frame, frame_end, 3):
                ...
                if codon in STOP_CODONS:
                    ...
                    segment_starts.append((position, codon_index, ambiguous_count))
                codon_index += 1
    ...
    def _summarize_segment(
        ...
        for start_position, start_codon_index, ambiguous_at_start in segment_starts:
            ...
            candidate_ambiguous_count = ambiguous_count - ambiguous_at_start
            if candidate_ambiguous_count and not self.allow_ambiguous:
                continue

            tx_start, tx_end, _orf_strand = self._scan_to_transcript_interval(
                tx=tx,
                sequence_length=sequence_length,
                source_strand=source_strand,
            ...
            summary[2] += aa_length + (1 if completeness == "complete" and self.include_stop else 0)
            if source_strand == "sense" and self._matches_annotated_interval(
                tx=tx,
                tx_start=tx_start,
                tx_end=tx_end,
            ):
    ...
    def _annotated_morf_spec(
        ...
        ambiguous_codon_count = self._ambiguous_codon_count(nucleotide_sequence)
    ...
    def _is_discarded_internal_orf(
        ...
        if tx_start == cds_start and tx_end == cds_end:
            return False
        return tx_start >= cds_start and tx_end <= cds_end and (tx_start - cds_start) % 3 == 0
    ...
    def _scan_frame(
        ...
        frame_peptide = "".join(GENETIC_CODE.get(codon, "X") for codon in codons)
    ...
    def _emit_complete_segment(
        ...
        for start_position, start_codon_index, ambiguous_at_start in segment_starts:
            ...
            peptide_sequence = frame_peptide[start_codon_index:peptide_end]
    ...
    def _create_record(
        ...
        tx_orf_start, tx_orf_end, orf_strand = self._scan_to_transcript_interval(
            tx=tx,
    ...
    def _build_record(
        ...
        if completeness == "complete":
            peptide_sequence = (
                translated_sequence if self.include_stop else translated_sequence[:-1]
            )
        else:
    ...
    def _ambiguous_codon_count(seq: str) -> int:
        ...
        return sum(
            seq[position : position + 3] not in GENETIC_CODE for position in range(0, len(seq), 3)
        )
