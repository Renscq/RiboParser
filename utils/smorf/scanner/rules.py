...
# Date: 2026-08-01
# Version: dev001
# Function: Classify and prioritize overlapping smORF candidates.
# Input: Transcript models and scanned ORF records.
# Output: ORF categories, overlap labels, and deterministic priorities.

"""Biological classification and overlap rules for scanned ORFs."""
...
from .reference import STOP_CODONS
...
class ORFOverlapMarker:
    ...
    def _mark_same_frame_overlap(records: list[ORFRecord]) -> None:
        ...
        for items in grouped.values():
            ...
            for primary in primaries:
                if any(
                    other is not primary and ORFOverlapMarker._is_overlap(primary, other)
                    for other in items
                ):
    ...
    def _mark_different_frame_overlap(
        ...
        for record in records:
            grouped[(record.transcript_id, record.source_strand)].append(record)
    ...
    def _downgrade_orf(
        ...
        if ORFOverlapMarker._is_identical(record, best):
            ...
            if record.start_codon != "ATG" and best.start_codon == "ATG":
    ...
    def _priority_key(record: ORFRecord) -> tuple:
        ...
        """
        annotated_rank = 0 if record.category in {"annotated_ORF", "annotated_mORF"} else 1
        discarded_rank = (
            1 if record.priority == "discarded" or record.category == "same_frame_iORF" else 0
        )
        completeness_rank = 0 if record.completeness == "complete" else 1
    ...
    def _kozak_score(record: ORFRecord) -> int:
        ...
        if 0 <= minus3_index < len(sequence) and sequence[minus3_index] in {"A", "G"}:
            score += 1
        if 0 <= plus4_index < len(sequence) and sequence[plus4_index] == "G":
            score += 1
        return score
    ...
    def _is_overlap(first: ORFRecord, second: ORFRecord) -> bool:
        ...
        return first.tx_orf_start < second.tx_orf_end and first.tx_orf_end > second.tx_orf_start

    @staticmethod
    def _is_nested(first: ORFRecord, second: ORFRecord) -> bool:
        """Return whether ``first`` is contained in ``second``."""
        return second.tx_orf_start <= first.tx_orf_start and second.tx_orf_end >= first.tx_orf_end

    @staticmethod
    def _is_identical(first: ORFRecord, second: ORFRecord) -> bool:
        """Return whether two transcript intervals are identical."""
        return first.tx_orf_start == second.tx_orf_start and first.tx_orf_end == second.tx_orf_end
