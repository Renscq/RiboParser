...
class GenePredWriter:
    ...
    def exon_frames(record: ORFRecord) -> list[int]:
        ...
        return [frame_by_block[block] for block in sorted(blocks, key=lambda block: block[0])]
    ...
    def format_record(record: ORFRecord) -> str:
        ...
        exon_frames = ",".join(map(str, GenePredWriter.exon_frames(record))) + ","
        completion = "cmpl" if record.completeness == "complete" else "incmpl"
        return "\t".join(
            [
                record.orf_id,
                record.chrom,
    ...
    def write(path: str | Path, records: list[ORFRecord]) -> None:
        ...
        with Path(path).open("w", encoding="utf-8") as handle:
            handle.write("".join(GenePredWriter.format_record(record) + "\n" for record in records))
...
class MessageWriter:
    ...
    def write(path: str | Path, records: list[ORFRecord]) -> None:
        ...
        with Path(path).open("w", encoding="utf-8") as handle:
            ...
            handle.write("".join(MessageWriter.format_record(record) + "\n" for record in records))
...
class FastaWriter:
    ...
    def write_nt(path: str | Path, records: list[ORFRecord]) -> None:
        ...
        with Path(path).open("w", encoding="utf-8") as handle:
            handle.write("".join(FastaWriter.format_nt(record) for record in records))

    @staticmethod
    def write_pep(path: str | Path, records: list[ORFRecord]) -> None:
        """Write peptide FASTA records."""
        with Path(path).open("w", encoding="utf-8") as handle:
            handle.write("".join(FastaWriter.format_pep(record) for record in records))
...
class ORFOutputWriter:
    ...
    def __init__(
        ...
        self.temporary_paths = {
            name: path.with_name(path.name + ".tmp") for name, path in self.final_paths.items()
        }
    ...
    def __enter__(self) -> ORFOutputWriter:
        ...
        try:
            ...
            if self.write_message_header:
                self.handles["message"].write("\t".join(MessageWriter.HEADER) + "\n")
        except Exception:
    ...
    def write_records(self, records: list[ORFRecord]) -> None:
        ...
        self.handles["genepred"].write(
            "".join(GenePredWriter.format_record(record) + "\n" for record in records)
        )
        self.handles["message"].write(
            "".join(MessageWriter.format_record(record) + "\n" for record in records)
        )
        self.handles["nt"].write("".join(FastaWriter.format_nt(record) for record in records))
        self.handles["pep"].write("".join(FastaWriter.format_pep(record) for record in records))
    ...
    def merge_parts(
        ...
        final_paths = {name: Path(f"{prefix}{suffix}") for name, suffix in suffixes.items()}
        temporary_paths = {
        ...
        try:
            ...
            handles["message"].write(("\t".join(MessageWriter.HEADER) + "\n").encode("utf-8"))
