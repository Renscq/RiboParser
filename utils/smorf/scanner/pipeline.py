...
from .engine import ORFScanner
from .models import ORFRecord, Transcript
from .output import (
    FastaWriter,
    GenePredWriter,
    MessageWriter,
...
from .reference import CoordinateMapper, FastaParser, GenePredParser
from .rules import ORFClassifier, ORFOverlapMarker
...
def _require_worker_resources() -> tuple[
    ...
    if _WORKER_GENOME is None or _WORKER_CONFIG is None or _WORKER_SCANNER is None:
...
def _scan_transcript(
    ...
    try:
        ...
        if config["remove_discarded"]:
            records = [record for record in records if record.priority != "discarded"]
        return records
...
def _summarize_transcript_worker(
    ...
    try:
        ...
        orf_count, nucleotide_length, peptide_length = scanner.summarize_transcript(
            transcript,
    ...
    with ORFOutputWriter(part_prefix) as output_writer:
        for _input_index, transcript, first_orf_index, expected_count in entries:
            ...
            for local_index, record in enumerate(records):
                record.orf_id = f"{config['orf_prefix']}{first_orf_index + local_index:08d}"
...
class SmORFPipeline:
    ...
    def _multi_process_results(
        ...
        message_print(
            f"Multiprocessing: workers={self.threads}, start_method={context.get_start_method()}, "
    ...
    def _build_part_tasks(
        ...
        for expected_index, (transcript, summary) in enumerate(
            ...
            if transcript.transcript_id != transcript_id or transcript.gene_id != gene_id:
                raise RuntimeError(
                    f"Transcript summary identity is inconsistent at index {expected_index}."
                )
    ...
    def _run_sharded_multiprocessing(
        ...
        message_print(
            f"Fast multiprocessing: workers={self.threads}, start_method={context.get_start_method()}, "
            f"summary_chunksize={chunksize}."
        )
        ...
        try:
            with ProcessPoolExecutor(
                ...
                summaries: list[TranscriptSummary] = []
                summary_interval = self._progress_interval(self.transcript_count)
                for completed, summary in enumerate(
                    summary_iterator,
                    start=1,
                ):
                    summaries.append(summary)
                    if completed == self.transcript_count or completed % summary_interval == 0:
                        progress_print(
                            f"indexed_transcripts={completed:,}/{self.transcript_count:,}."
                        )
                ...
                message_print(f"Balanced output partitions: {len(part_tasks):,}.")
                ...
                for completed, result in enumerate(
                    ...
                    progress_print(
                        f"output_parts={completed:,}/{len(part_tasks):,}, "
                        f"last_gene={last_gene}, last_transcript={last_transcript}, "
                        f"total_ORFs={observed_orf_count:,}."
                    )
        ...
        with ORFOutputWriter(self.out_prefix) as output_writer:
            for completed, (
                ...
                for record in transcript_records:
                    record.orf_id = f"{self.orf_prefix}{next_orf_index:08d}"
                    next_orf_index += 1
                ...
                if (
                    ...
                    progress_print(
                        f"transcripts={completed:,}/{self.transcript_count:,}, "
                        f"last_gene={gene_id}, last_transcript={transcript_id}, "
                        f"total_ORFs={self.orf_count:,}."
                    )
