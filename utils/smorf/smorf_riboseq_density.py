...
from .smorf_riboseq_constants import SUPPORTED_DENSITY_FORMATS
...
class ChromDensity:
    ...
    def query(self, start: int, end: int) -> np.ndarray:
        ...
        if start < 0 or end < start:
            raise ValueError(
                f"Invalid density query interval: [{start}, {end})."
            )
        if end == start:
...
def infer_density_format(
    ...
    if user_format and user_format != "auto":
        ...
        if format_name not in SUPPORTED_DENSITY_FORMATS:
            raise ValueError(
                f"Unsupported density format: {format_name}"
            )
        return format_name

    lower = str(path).lower()
    if lower.endswith(
        (".bedgraph", ".bdg", ".bedgraph.gz", ".bdg.gz")
    ):
        return "bedgraph"
    if lower.endswith(
        (".wig", ".wiggle", ".wig.gz", ".wiggle.gz")
    ):
        return "wig"
    raise ValueError(
        "Cannot infer density format from filename. "
        "Use --density-format wig or bedgraph."
    )
...
def bedgraph_needs_sort(path: str | Path) -> bool:
    ...
    for line_number, text in _iter_bedgraph_data_lines(path):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid bedGraph coordinates at line {line_number}."
            ) from error
        ...
        if start < last_start or (
            start == last_start and end < last_end
        ):
            return True
        last_start = start
        last_end = end
...
def sort_bedgraph(
    ...
    try:
        ...
        if return_code != 0:
            raise RuntimeError(
                "Automatic bedGraph sorting failed: "
                + stderr.strip()
            )
...
def _validate_interval(
    ...
    if not chrom:
        raise ValueError(
            f"Empty chromosome at density line {line_number}."
        )
    if start < 0 or end <= start:
        raise ValueError(
            f"Invalid density interval at line {line_number}: "
            f"{chrom}:{start}-{end}"
        )
...
def _iter_bedgraph(path: str | Path) -> Iterator[ChromDensity]:
    ...
    for line_number, text in _iter_bedgraph_data_lines(path):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid bedGraph numeric value at line "
                f"{line_number}."
            ) from error
    ...
    with smart_open(path, "rt") as handle:
        for line_number, raw_line in enumerate(handle, start=1):
            ...
            if text.startswith("fixedStep"):
                ...
                if not chrom:
                    raise ValueError(
                        f"fixedStep header lacks chrom at line "
                        f"{line_number}."
                    )
                if current_chrom is None:
                    current_chrom = chrom
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid fixedStep header at line "
                        f"{line_number}."
                    ) from error
                if position < 0 or step < 1 or span < 1:
                    raise ValueError(
                        f"Invalid fixedStep coordinates at line "
                        f"{line_number}."
                    )
                mode = "fixed"
                continue
            ...
            if text.startswith("variableStep"):
                ...
                if not chrom:
                    raise ValueError(
                        f"variableStep header lacks chrom at line "
                        f"{line_number}."
                    )
                if current_chrom is None:
                    current_chrom = chrom
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid variableStep header at line "
                        f"{line_number}."
                    ) from error
                if span < 1:
                    raise ValueError(
                        f"Invalid variableStep span at line "
                        f"{line_number}."
                    )
                mode = "variable"
                position = None
            ...
            if current_chrom is None or mode is None:
                raise ValueError(
                    f"WIG value occurs before a step header at line "
                    f"{line_number}."
                )
            ...
            if mode == "fixed":
                if position is None:
                    raise ValueError(
                        f"Missing fixedStep position at line "
                        f"{line_number}."
                    )
                try:
                    ...
                    raise ValueError(
                        f"Invalid WIG value at line {line_number}."
                    ) from error
                start = position
                end = position + span
                position += step
                ...
                if len(fields) < 2:
                    raise ValueError(
                        f"Invalid variableStep line {line_number}."
                    )
                try:
                    ...
                    raise ValueError(
                        f"Invalid variableStep value at line "
                        f"{line_number}."
                    ) from error
                end = start + span
